...
use rusttype::{gpu_cache::Cache, Font, Glyph, Scale, Point};
use slab::Slab;
use std::collections::HashMap;

use crate::{
    context::{Align, ImageFlags, ImageId, TextMetrics},
    math::{Bounds, Extent},
    renderer::{Renderer, TextureType},
};

use super::{FontData, FontId, LayoutChar};
            ...
            if cache {
                ...
                for lc in result {
                    if let Ok(Some((uv, _))) = self.cache.rect_for(lc.id, &lc.glyph) {
                        lc.uv = Bounds {
                            min: crate::math::Point {
                                x: uv.min.x,
                                y: uv.min.y,
                            },
                            max: crate::math::Point {
                                x: uv.max.x,
                                y: uv.max.y,
                            },
                        };
